...
resource "aws_subnet" "private" {
  ...
  availability_zone = data.aws_availability_zones.available.names[count.index]
  tags = {
    Name = "${var.app}-${var.environment}-private-${count.index + 1}"
  }
}
...
resource "aws_subnet" "secure" {
  ...
  availability_zone = data.aws_availability_zones.available.names[count.index]
  tags = {
    Name = "${var.app}-${var.environment}-secure-${count.index + 1}"
  }
}
...
resource "aws_route_table" "public" {
  vpc_id = aws_vpc.vpc.id
  tags = {
    Name = "${var.app}-${var.environment}-public-rt"
  }
}
...
resource "aws_route_table" "private" {
  vpc_id = aws_vpc.vpc.id
  tags = {
    Name = "${var.app}-${var.environment}-private-rt"
  }
}
...
resource "aws_route_table" "secure" {
  vpc_id = aws_vpc.vpc.id
  tags = {
    Name = "${var.app}-${var.environment}-secure-rt"
  }
}
...
resource "aws_network_acl" "main" {
  ...
  tags = {
    Name = "${var.app}-${var.environment}-nacl"
  }
}
